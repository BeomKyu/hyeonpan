...
NOW 2026-03-08
$Hyeonpan.KanbanBoard: v3.0

# === 프로젝트 개요 ===
# 노션 스타일 칸반보드 + 대화형 히스토리 뷰어.
# 카드 클릭 시 채팅 형식 타임라인 + diff 표시.
# 데이터 어댑터 패턴: 수동입력(MVP) / API연동(미래) 전환 가능.
# @dnd-kit 기반 드래그앤드롭 (카드 이동 + 칼럼 순서 변경).
...
[_] 단위: $Board.Actions.add_column 칼럼_추가_정상_동작
[_] 단위: $Board.Actions.delete_column 칼럼_삭제_시_하위_카드_연쇄_제거
[_] 단위: $Board.Actions.delete_card 카드_삭제_시_칼럼_card_ids_정리
[_] 단위: $Board.Actions.move_card 카드_이동_후_from/to_칼럼_정합성
[_] 단위: $Board.Actions.reorder_columns 칼럼_순서_변경_후_order_재정렬
[_] 단위: $Board.Actions.add_message 메시지_추가_후_messages_배열_검증
[_] 단위: $Card.DiffBlock diff_줄별_파싱_및_색상_분류
[_] 단위: $ManualAdapter.load_board JSON_파싱_실패_시_기본값_반환
[_] 통합: 드래그앤드롭_카드_이동_→_상태_→_localStorage_동기화
[_] 통합: 칼럼_드래그앤드롭_순서_변경_→_order_필드_반영
[_] 통합: 카드_클릭_→_대화_타임라인_→_메시지_추가_→_저장
[_] 통합: 새로고침_후_보드_및_메시지_히스토리_완전_복원
[_] 통합: VITE_ADAPTER_MODE_전환_시_어댑터_경로_변경_검증
